      ...
      candidate_categories %>% full_join(candidate_total) %>% 
       ...
       # Now to get the percentages:
        ...
        # This worked well but I need to re-work it to only look at mayoral and city council 
            ...
            #  When trying to mutate that column, the percentage gets all sorts of fucked up for no reason despite working above. going to try to export 
            
            mayor_spend_perc %>% write_csv("mayor_spend_perc.csv", na = "")  
            
              # Looks right on export but not when I try to bring it back in, R can go fuck a duck 
